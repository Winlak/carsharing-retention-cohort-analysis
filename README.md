...
Полный ответ с цифрами текущего воспроизводимого прогона — в [аналитическом отчёте](reports/analytical_report.md). Для интерактивного разбора кода и визуализаций — [ноутбук](notebooks/carsharing_retention_case.ipynb).
...
4. исполняет ноутбук сверху вниз и канонизирует run-specific metadata: таблицы и текстовые outputs сохраняются, а inline-копии графиков убираются (канонические SVG уже лежат в `figures/`);
